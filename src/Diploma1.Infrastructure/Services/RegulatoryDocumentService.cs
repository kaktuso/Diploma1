...
namespace Diploma1.Infrastructure.Services
{
    ...
    {
        private readonly IRegulatoryDocumentRepository _repo;
        private readonly IUnitOfWork _uow;
        private readonly IMapper _mapper;
        public RegulatoryDocumentService(IRegulatoryDocumentRepository repo, IUnitOfWork uow, IMapper mapper)
        {
            _repo = repo;
            _uow = uow;
            _mapper = mapper;
        ...
        public async Task<RegulatoryDocumentDto?> GetByIdAsync(Guid id)
        {
            ...
            return entity == null ? null : _mapper.Map<RegulatoryDocumentDto>(entity);
        }
        public async Task<IEnumerable<RegulatoryDocumentDto>> GetAllAsync()
        {
            var entities = await _repo.GetAllAsync();
            return _mapper.Map<IEnumerable<RegulatoryDocumentDto>>(entities);
        }
        public async Task<RegulatoryDocumentDto> CreateAsync(RegulatoryDocumentDto dto)
        {
            var entity = _mapper.Map<RegulatoryDocument>(dto);
            await _repo.AddAsync(entity);
            await _uow.SaveChangesAsync();
            return _mapper.Map<RegulatoryDocumentDto>(entity);
        }
        public async Task UpdateAsync(RegulatoryDocumentDto dto)
        {
            var entity = _mapper.Map<RegulatoryDocument>(dto);
